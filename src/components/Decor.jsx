...
// //   const { data: array, isLoading ,error} = useQuery({
// //     queryKey: ["decorData"],
...
// //       <Navbar setShowSearchGlobal={setShowSearchGlobal} />
// //       {!openForm && (
// //         <button
// //           onClick={() => setOpenForm(true)}
// //           className="bg-blue-500 absolute right-3 m-4 text-white font-bold py-2 px-4 rounded-lg shadow-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400"
// //         >
// //           Add Product
// //         </button>
// //       )}
...
// //           <Link 
// //   to={`/decor/${index}`} 
// //   state={{ product: data }}  // Pass the individual product instead of the array
// // >
...
//     const matchesMinPrice = minPrice === "" || item.cost >= parseFloat(minPrice);
//     const matchesMaxPrice = maxPrice === "" || item.cost <= parseFloat(maxPrice);
...
//   return (
//     <div className="relative">
//       <Navbar setShowSearchGlobal={setShowSearchGlobal} onSearch={handleSearch} />
//       {!openForm && (
//         <button
//           onClick={() => setOpenForm(true)}
//           className="bg-blue-500 absolute right-3 m-4 text-white font-bold py-2 px-4 rounded-lg shadow-md hover:bg-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-400"
...
export const Decor = () => {
  ...
  const { data: array, isLoading, error, refetch, isFetching } = useQuery({
    queryKey: ["decorData"],
    queryFn: fetchDecorData
  });
    ...
    <div className="relative">
      ...
      <div className="flex justify-end gap-4 items-center px-4 py-2">
        ...
        <button
          ...
          disabled={isFetching}
        >
          <RefreshCw className={`mr-2 h-5 w-5 ${isFetching ? 'animate-spin' : ''}`} />
          Refresh
        </button>
      </div>
      <div className="text-2xl font-bold text-green-500 h-[10vh] items-center sm:text-3xl sm:leading-relaxed flex justify-center">Decor</div>
